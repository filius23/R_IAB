...
-   Erstellen Sie eine kompaktere Version des Datensatzes (bspw. als `pend_small`) mit lediglich den Variablen `pnr`, `hnr`, `welle`, `pintjahr` und `palter`.
...
-  Exportieren Sie den `data.frame` mit den in der vorherigen Übung erstellten kleineren Datensatz-Version (`pend_small`) als `.Rdata`-Datei.
-  Laden Sie die eben exportierte `.Rdata`-Datei unter einem beliebigen Namen, bspw. `pend_neu`. 
-  Vergleichen Sie das neu gelandene mit dem ursprünglichen Objekt: `identical(pend_1_small,pend_neu)`
