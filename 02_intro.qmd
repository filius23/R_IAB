...
Am IAB existieren einige Workarounds, aktuell habe ich am FDZ eine Lösung in Arbeit, die den Befehl `fdz_install()` zur Verfügung stellt - dieser verhält sich wie der Standard-Befehl `install.packages()` (oder sollte es zumindest).

Legt die `.Rprofile`-Datei in `C:\Users\*EUREKENNUNG*\Documents` und startet R neu (`STRG + F10`)
